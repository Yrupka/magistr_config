...
public class Profile_system_lab_2 : MonoBehaviour
{
    private List<Engine_options_lab_2> options;
    private int choosed_profile;
    public Options_lab_2 engine_options;
    ...
    {
        (options, choosed_profile) = File_controller.Load_lab_2();
        if (options != null)
        {
            foreach (Engine_options_lab_2 item in options)
                Item_add(item.car_name + "-" + item.engine_name);
            Data_update(choosed_profile);
        }
        else
            options = new List<Engine_options_lab_2>();
    }
