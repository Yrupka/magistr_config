...
public class Profile_system_lab_3 : MonoBehaviour
{
    private List<Engine_options_lab_3> options;
    private int choosed_profile;
    public Options_lab_3 engine_options;
    ...
    {
        (options, choosed_profile) = File_controller.Load_lab_3();
        if (options != null)
        {
            foreach (Engine_options_lab_3 item in options)
                Item_add(item.car_name + "-" + item.engine_name);
            Data_update(choosed_profile);
        }
        else
            options = new List<Engine_options_lab_3>();
    }
    ...
    private void Profile_add()
    {
        ...
        options.Add(new Engine_options_lab_3(name_engine, name_car));
    }
